  ...
  **/
  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        ...
        if (s != "") {
          ...
          // Compute steer error 
          int n = x_points.size();
          double x_final = x_points[n-1];
          double y_final = y_points[n-1];
          double target_yaw = atan2(y_final-y_position, x_final-x_position);
          ...
          int m = v_points.size();
          double error_throttle = v_points[m-1] - velocity;
